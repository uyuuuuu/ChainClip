...
export default function DoneScreen() {
    ...
    // プレーヤーの状態をUIに反映
        useEffect(() => {
            const subs = [
                player.addListener('playingChange', ({ isPlaying }) => setIsPlaying(isPlaying)),
        ...
        // シークバーの進捗（0〜100）
        const progressValue = durationMs > 0 ? Math.min((currentMs / durationMs) * 100, 100) : 0;
        ...
        const handleSave = async () => {
            ...
            try {
                ...
                if (!granted) {
                    Alert.alert('保存できません', '設定から写真へのアクセスを許可してください。');
                    return;
                }
    ...
    return(
           ...
           {status === 'completed' ? (
            ...
            <View className="items-center">
                <View className="bg-black overflow-hidden rounded-xl"
                ...
                <View className="mx-6 mt-2">
                    <Text className="text-xl font-bold text-[#262626] numberOfLines={1}">{title}</Text>
                </View>
